# Keyboard Controller（キーボード操作ノード）

キーボード入力を `geometry_msgs/msg/Twist` メッセージに変換し、`/cmd_vel` トピックへ
**20 Hz** で発行するノードです。記録済み JSON シナリオの再生にも対応し、再現性のある
デモを実行できます。
...
| 項目 | 値 |
|------|----|
| ノード名 | `keyboard_controller` |
| トピック | `/cmd_vel` (`geometry_msgs/msg/Twist`) |
| レート | 20 Hz |
...
sequenceDiagram
    ...
    participant PUB as 発行ループ<br/>publish_loop (20Hz)
    ...
    loop 1/20 秒ごと
        PUB->>ST: _make_twist() で現在速度を読む
        PUB->>CL: /cmd_vel (Twist) を発行
    end

    U->>LSN: Q / ESC
    LSN->>PUB: _shutdown() → ループ終了 + ゼロ速度を最終発行
```
...
| `dry_run` | bool | False | ROS なしで動作（rclpy 不在時も自動的に True） |
...
| `_stopping()` | なし | シャットダウン要求 or `rclpy` 停止を検知してループ中断に使う |

#### 発行・ライフサイクル

| 関数 | 引数 | 説明 |
|------|------|------|
| `_make_twist()` | なし | 現在速度から `Twist` を生成（`linear.x` と `angular.z` のみ非ゼロ） |
| `publish_loop()` | なし | 20 Hz で `Twist` を発行。終了時にゼロ速度を最終発行。dry-run 時はアイドル |
| `_shutdown()` | なし | 多重防止つき終了処理。リスナー停止・ノード破棄・`rclpy.shutdown()` |
...
コマンドライン引数を解釈。`--ros-args ...` を除去してから解釈し、未知引数は警告。
...
`--dry-run`（`rclpy` が import できない場合も自動的に有効）は ROS パブリッシャを
省略するため、どのマシンでもキー処理・シナリオ解析を確認できます。
...
1. バックエンドサービスを起動（`run_keyboard.sh` / `.ps1` または `docker compose up -d`）。
2. 別端末で `/cmd_vel` を観測:
   ```bash
