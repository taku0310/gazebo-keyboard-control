...
ROS Noetic + Docker + Gazebo (Ignition Fortress) keyboard-control robot demo,
...
2. **Resource & lifecycle** — threads/subscribers/publishers started but never
   ...
   cleanly on Ctrl+C / `rospy.signal_shutdown`).
...
4. **Consistency** — match the established style: emoji debug output, throttled
   logging, lazy optional imports (`pynput`, `rospy`) so code stays testable
   headless, `--dry-run` paths.

## Project-specific gotchas to watch for

- **Headless testability**: top-level imports of `pynput`/`rospy` must stay
